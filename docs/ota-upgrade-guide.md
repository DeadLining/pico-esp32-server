...
在单模块部署中，pico-server内置了OTA固件管理功能，可以自动检测设备版本并下发升级固件。系统会根据设备型号和当前版本，自动匹配并推送最新的固件版本。

## 前提条件

- 你已经成功进行**单模块部署**并运行pico-server
- 设备能够正常连接到服务器
...
重要的事情说三遍：升级的bin文件是`pico.bin`，不是全量固件文件`merged-binary.bin`!
...
```bash
cp pico.bin data/bin/设备型号_版本号.bin
```

例如：
```bash
cp pico.bin data/bin/bread-compact-wifi_1.6.6.bin
```
...
如果你的pico-server是公网部署（使用公网IP或域名），**必须**配置`server.vision_explain`参数，因为OTA固件下载地址会使用该配置的域名和端口。
