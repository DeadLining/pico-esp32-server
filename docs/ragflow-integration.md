...
如果您对ragflow很熟悉，且已经部署了ragflow，可直接跳过第一部分，直接进入第二部分。但是如果你希望有人指导你部署ragflow，让它能够和`pico-esp32-server`共同使用`mysql`、`redis`基础服务，以减少资源成本，你需要从第一部分开始。
...
注意是将`pico-esp32-server-db`和`pico-esp32-server-redis`下面的`expose`改成`ports`。改完后，需要重新启动。以下是重启mysql的命令：

``` shell
# 进入你docker-compose_all.yml所在的文件夹，例如我的是pico-server
cd pico-server
...
你需要在你电脑找一个文件夹，用来存放ragflow项目。例如我在`/home/system/pico`文件夹。
