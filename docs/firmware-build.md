...
```
http://192.168.1.25:8003/pico/ota/
```
如果显示“OTA接口运行正常，向设备发送的websocket地址是：ws://xxx:8000/pico/v1/
...
此刻，请你用浏览器打开你的ota地址，例如我的ota地址
```
http://192.168.1.25:8002/pico/ota/
```
...
```
ws://192.168.1.25:8000/pico/v1/
```

配置完后，再使用浏览器刷新你的ota接口地址，看看是不是正常了。如果还不正常就，就再次确认一下Websocket是否正常启动，是否配置了Websocket地址。

## 第2步 配置环境
先按照这个教程配置项目环境[《Windows搭建 ESP IDF 5.3.2开发环境以及编译Pico》](https://icnynnzcwou8.feishu.cn/wiki/JEYDwTTALi5s2zkGlFGcDiRknXf)

## 第3步 打开配置文件
配置好编译环境后，下载虾哥iaozhi-esp32项目源码，

从这里下载虾哥[pico-esp32项目源码](https://github.com/78/xiaozhi-esp32)。

下载后，打开`pico-esp32/main/Kconfig.projbuild`文件。

## 第4步 修改OTA地址

找到`OTA_URL`的`default`的内容，把`https://api.tenclass.net/pico/ota/`
   改成你自己的地址，例如，我的接口地址是`http://192.168.1.25:8002/pico/ota/`，就把内容改成这个。

修改前：
```
config OTA_URL
    string "Default OTA URL"
    default "https://api.tenclass.net/pico/ota/"
    help
...
config OTA_URL
    string "Default OTA URL"
    default "http://192.168.1.25:8002/pico/ota/"
    help
        The application will access this URL to check for new firmwares and server address.
```
...
进入菜单配置后，再进入`Pico Assistant`，将`BOARD_TYPE`设置你板子的具体型号
保存退出，回到终端命令行。
...
烧录成功且联网成功后，通过唤醒词唤醒Pico，留意server端输出的控制台信息。

## 常见问题
以下是一些常见问题，供参考：

[1、为什么我说的话，Pico识别出来很多韩文、日文、英文](./FAQ.md)
...
[5、如何提高Pico对话响应速度？](./FAQ.md)

[6、我说话很慢，停顿时Pico老是抢话](./FAQ.md)

[7、我想通过Pico控制电灯、空调、远程开关机等操作](./FAQ.md)
