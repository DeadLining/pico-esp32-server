...
`pico-esp32-server`项目，可结合虾哥开源的[pico-mqtt-gateway](https://github.com/78/pico-mqtt-gateway) 项目进行简单改造，即可实现Pico硬件MQTT+UDP连接。
本教程分为三部分，你可以根据你是全模块部署还是单模块部署，选择对应的部分接入MQTT网关：
- 第一部分：部署MQTT网关
- 第二部分：全模块运行实现Pico硬件MQTT+UDP连接
- 第三部分：单模块运行pico-server实现Pico硬件MQTT+UDP连接

## 准备阶段
准备好你的`pico-server`的`mqtt-websocket`连接地址。在你原来的`websocket地址`基础上，添加`?from=mqtt_gateway`字符，就可以得到`mqtt-websocket`连接地址

1、如果你是源码部署，你的`mqtt-websocket`地址是：
```
ws://127.0.0.1:8000/pico/v1/?from=mqtt_gateway
```

2、如果你是docker部署，你的`mqtt-websocket`地址是
```
ws://你宿主机局域网IP:8000/pico/v1/?from=mqtt_gateway
```
...
1. 克隆[改造后的pico-mqtt-gateway项目](https://github.com/xinnan-tech/pico-mqtt-gateway.git)：
```bash
git clone https://ghfast.top/https://github.com/xinnan-tech/pico-mqtt-gateway.git
cd pico-mqtt-gateway
```
...
4. 编辑配置文件 config/mqtt.json，把你在`本文准备阶段`的`mqtt-websocket`地址替换到`chat_servers`里。例如源码部署的`pico-server`就是如下配置：
...
SERVER_SECRET=Te1st12134  # 服务器密钥，请保持和智控台（server.secret）一致或者和pico-server里（server.auth_key）保持一致
```
...
上面的配置完成后，你可以使用curl命令，验证你的ota地址是否会下发mqtt配置，把下面的`http://localhost:8002/pico/ota/`改成你的ota地址
```
curl 'http://localhost:8002/pico/ota/' \
...
```
{"server_time":{"timestamp":1757567894012,"timeZone":"Asia/Shanghai","timezone_offset":480},"activation":{"code":"460609","message":"http://pico.server.com\n460609","challenge":"11:22:33:44:55:66"},"firmware":{"version":"1.0.1","url":"http://pico.server.com:8002/pico/otaMag/download/NOT_ACTIVATED_FIRMWARE_THIS_IS_A_INVALID_URL"},"websocket":{"url":"ws://192.168.4.23:8000/pico/v1/"},"mqtt":{"endpoint":"192.168.0.7:1883","client_id":"GID_default@@@11_22_33_44_55_66@@@7b94d69a-9808-4c59-9c9b-704333b38aff","username":"eyJpcCI6IjA6MDowOjA6MDowOjA6MSJ9","password":"Y8XP9xcUhVIN9OmbCHT9ETBiYNE3l3Z07Wk46wV9PE8=","publish_topic":"device-server","subscribe_topic":"devices/p2p/11_22_33_44_55_66"}}
```
...
如果返回的内容包含`mqtt`相关的配置，说明配置成功。类似这样
```
{"server_time":{"timestamp":1758781561083,"timeZone":"GMT+08:00","timezone_offset":480},"activation":{"code":"527111","message":"http://pico.server.com\n527111","challenge":"11:22:33:44:55:66"},"firmware":{"version":"1.0.1","url":"http://pico.server.com:8002/pico/otaMag/download/NOT_ACTIVATED_FIRMWARE_THIS_IS_A_INVALID_URL"},"websocket":{"url":"ws://192.168.1.15:8000/pico/v1/"},"mqtt":{"endpoint":"192.168.1.15:1883","client_id":"GID_default@@@11_22_33_44_55_66@@@11_22_33_44_55_66","username":"eyJpcCI6IjE5Mi4xNjguMS4xNSJ9","password":"fjAYs49zTJecWqJ3jBt+kqxVn/x7vkXRAc85ak/va7Y=","publish_topic":"device-server","subscribe_topic":"devices/p2p/11_22_33_44_55_66"}}
```
