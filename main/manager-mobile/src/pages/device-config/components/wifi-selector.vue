...
async function checkESP32Connection() {
  ...
  try {
    ...
    console.log(`${t('deviceConfig.pico')}连接状态:`, isConnectedToESP32.value)
  }
  catch (error) {
    isConnectedToESP32.value = false
    emit('connection-status', false)
    console.log('pico连接检查失败:', error)
  }
  finally {
      ...
      <view v-if="!isConnectedToESP32" class="status-warning">
        <view class="status-content">
          <text class="warning-text">
            {{ t('deviceConfig.connectPicoHotspot') }} (pico-XXXXXX)
          </text>
          <wd-button
            size="small"
            type="primary"
      ...
      <view v-else class="status-success">
        <view class="status-content">
          <text class="success-text">
            {{ t('deviceConfig.connectedPicoHotspot') }}
          </text>
          <wd-button
            size="small"
            :loading="checkingConnection"
