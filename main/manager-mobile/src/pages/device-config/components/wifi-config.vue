...
async function submitConfig() {
  ...
  if (!connected) {
    toast.error(t('deviceConfig.connectPicoHotspot'))
    return
  }
  ...
  try {
    ...
    if (response.statusCode === 200 && (response.data as any)?.success) {
      toast.success(`${t('deviceConfig.configSuccess')}！${t('deviceConfig.deviceWillConnectTo')} ${props.selectedNetwork.ssid}，${t('deviceConfig.deviceWillRestart')}。${t('deviceConfig.pleaseDisconnectPicoHotspot')}`)
      // 设备退出配网模式
      setTimeout(() => {
        uni.request({
    ...
    <view class="help-section">
      ...
      <view class="help-content">
        <text class="help-item">
          1. {{ t('deviceConfig.phoneConnectPicoHotspot') }} (pico-XXXXXX)
        </text>
        <text class="help-item">
