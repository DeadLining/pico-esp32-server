...
import pico.common.utils.AESUtils;
import pico.common.utils.JsonUtils;
import pico.modules.agent.Enums.PicoMcpJsonRpcJson;
    ...
    public List<String> getAgentMcpToolsList(String id) {
        ...
        try {
            ...
            try (WebSocketClientManager client = WebSocketClientManager.build(
                ...
                client.sendText(PicoMcpJsonRpcJson.getInitializeJson());
                ...
                client.sendText(PicoMcpJsonRpcJson.getNotificationsInitializedJson());
                // 步骤3: 发送工具列表请求 - 立即发送，无需额外延迟
                log.info("发送MCP工具列表请求，智能体ID: {}", id);
                client.sendText(PicoMcpJsonRpcJson.getToolsListJson());
