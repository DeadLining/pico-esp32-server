...
export const register = () => {
  if (process.env.NODE_ENV === 'production' && 'serviceWorker' in navigator) {
    window.addEventListener('load', () => {
      ...
      console.info(`[Pico服务] 正在尝试注册Service Worker，URL: ${swUrl}`);
      
      // 先检查Service Worker是否已注册
      navigator.serviceWorker.getRegistrations().then(registrations => {
        if (registrations.length > 0) {
          console.info('[Pico服务] 发现已有Service Worker注册，正在检查更新');
        }
        ...
        navigator.serviceWorker
          .register(swUrl)
          .then(registration => {
            console.info('[Pico服务] Service Worker注册成功');
              ...
              installingWorker.onstatechange = () => {
                if (installingWorker.state === 'installed') {
                  if (navigator.serviceWorker.controller) {
                    // 内容已缓存更新，通知用户刷新
                    console.log('[Pico服务] 新内容可用，请刷新页面');
                    // 可以在这里展示更新提示
          ...
          .catch(error => {
            ...
            if (error.name === 'TypeError' && error.message.includes('Failed to register a ServiceWorker')) {
              console.warn('[Pico服务] 注册Service Worker时出现网络错误，CDN资源可能无法缓存');
